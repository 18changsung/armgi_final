...
class MainTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let tableViewWidth:CGFloat = CGFloat(tableView.frame.size.width)
        studyCell.goalStateBarBack.backgroundColor = UIColor().colorFromHex("#DFE5F0")
        studyCell.goalStateBarBack.frame.size.width = CGFloat(Float(tableView.frame.size.width) - 30)
        let edgeInsets = (tableViewWidth - studyCell.goalStateBarBack.frame.size.width) / 2
        
        let goalVal = dataCenter.goalData.currentGoalVal[indexPath.section]/dataCenter.goalData.goalList[indexPath.section]
        if goalVal <= 1.0{
            studyCell.goalStateBar.frame.size.width = CGFloat(goalVal * (Float(tableView.frame.size.width) - 30))
    ...
    override func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        let view = UIView()
        view.backgroundColor = UIColor().colorFromHex("#48bdf7")
    ...
    override func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        ...
        let edit = UITableViewRowAction(style: .normal, title: "수정") { (action, indexPath) in
            dataCenter.edit = true
            dataCenter.addSubject = indexPath.section
