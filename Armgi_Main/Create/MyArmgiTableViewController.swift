...
class MyArmgiTableViewController: UITableViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        for i in 0 ..< dataCenter.studyList.count {
            for j in 0 ..< dataCenter.studyList[i].unitList.count {
                for k in 0 ..< dataCenter.studyList[i].unitList[j].allWords.count {
                    allList.append("\(dataCenter.studyList[i].unitList[j].allWords[k].keyword)\r\n\(dataCenter.studyList[i].unitList[j].allWords[k].explanation)")
                }
            }
        }

        // 랜덤으로 모든 암기(all List에서) 중 하나 선정해서 보여준다.
        ...
        if allList.count > 0 && todayDate != currentDate { // 근데 만약 완전 처음 앱을 실행했다면? 아무것도 안뜨다가 다음날에 뜰것.
            let randNum:UInt32 = arc4random_uniform(UInt32(allList.count))
            randomTV.text = allList[Int(randNum)]
